...
export default function PomodoroTimer({ expiryTimestamp }: ITimeStampProps) {
  ...
  const restart = () => {
    ...
    const sessionDuration = phase === "work" ? 45 : 15;
    newExpiryTimestamp.setMinutes(
      newExpiryTimestamp.getMinutes() + sessionDuration
    );
    setIsRunning(true);
    setRemainingTime(
      (newExpiryTimestamp.getTime() - new Date().getTime()) / 1000
    );
  };
      ...
      timerRef.current = setInterval(() => {
        setRemainingTime(prevRemainingTime => {
          if (prevRemainingTime <= 1) {
            clearInterval(timerRef.current!);
            setIsRunning(false);
            setPhase(prevPhase => (prevPhase === "work" ? "break" : "work"));
            const newExpiryTimestamp = new Date();
            const sessionDuration = phase === "work" ? 15 : 45;
            newExpiryTimestamp.setMinutes(
              newExpiryTimestamp.getMinutes() + sessionDuration
            );
            setRemainingTime(
